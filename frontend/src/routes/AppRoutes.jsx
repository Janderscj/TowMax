...
function useGlobalSignOut(resetAll) {
  const { signOut } = useLookup();
...
function GarageDetailsWrapper() {
  ...
  const navigate = useNavigate();
  const handleGlobalSignOut = useGlobalSignOut(resetAll);
...
function VinBreakdownWrapper() {
  const { decoded, vin, resetAll } = useLookup();
  const { isGuest } = useAuth();
